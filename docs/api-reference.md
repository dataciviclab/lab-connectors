...
Wrapper su `concurrent.futures.ThreadPoolExecutor` per richieste HTTP parallele.
**Non esportato dalla superficie pubblica** — non più raccomandato per nuovi
consumer. Usa direttamente `concurrent.futures.ThreadPoolExecutor` con `HttpClient`.
...
| `CircuitOpenError` | Restituito in `HttpResult.err` se il circuit breaker è aperto |
...
Ogni log include tool name, messaggio e metadati strutturati.
...
Client GCS unificato per list, upload e verifica. 3 modalità:

- `auth=None` (default): prova SDK `google.cloud.storage`, fallback HTTP API pubblica
- `auth=True`: richiede SDK autenticato, fallisce con `RuntimeError` se assente
...
`object_exists()` non richiede il SDK (solo HTTP API). `upload_file()` e
`upload_string()` richiedono SDK autenticato.

### Path contract GCS (`lab_connectors.gcs.paths`)

Il modulo `paths` definisce i **path canonici** di tutti gli artifact su GCS del
DataCivicLab. I pattern sono caricati da `paths.json` e coprono 2 bucket e 8 pattern.
...
Imposta `memory_limit='2GB'` e `PRAGMA disable_progress_bar`.
...
Context manager per leggere parquet da GCS:
- URL `https://storage.googleapis.com/...` letti nativamente (senza estensioni, stabili);
- path `gs://...` o `s3://...` — inclusi glob multi-year (`gs://bucket/slug/*/*.parquet`) — via estensione `httpfs` + config GCS.

```python
from lab_connectors.duckdb import gcs_connect

# HTTPS nativo (stabile, senza estensioni)
with gcs_connect("https://storage.googleapis.com/dataciviclab-clean/slug/2024/file.parquet") as con:
    result = con.execute("SELECT count(*) FROM read_parquet('https://...')").fetchone()

# Glob multi-year su gs:// (httpfs)
with gcs_connect("gs://dataciviclab-clean/slug/*/*.parquet") as con:
    result = con.execute("SELECT count(*) FROM read_parquet('gs://...', union_by_name=True)").fetchone()
```
...
risposte pre-configurate, senza monkeypatch o mock artigianali.
...
`FakeHttpClient` rispecchia l'interfaccia di `HttpClient` (`.get()`, `.head()`,
`.post()`, `.close()`, context manager) ma non fa rete. Le risposte possono
essere valori fissi o callable `(url, **kwargs) -> HttpResult`.
